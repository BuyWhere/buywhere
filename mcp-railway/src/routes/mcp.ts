  ...
  // pick which version to call. v1 handlers are reused unchanged.
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        domain: { type: 'string', description: 'Filter by merchant platform (e.g. lazada, shopee, amazon)' },
        region: { type: 'string', description: 'Filter by region (sea, us, eu, au)' },
        deliver_to: { type: 'string', description: "REQUIRED. Buyer's ISO country code (e.g. \"SG\", \"US\"). Scopes the search to products deliverable to that market.", minLength: 2 },
        country_code: { type: 'string', enum: ['SG', 'US', 'VN', 'TH', 'MY'], description: 'Filter by ISO country code.' },
        country: { type: 'string', description: 'Alias for country_code (deprecated, use country_code)' },
        min_price: { type: 'number', description: 'Minimum price (currency inferred from deliver_to, default SGD)' },
        max_price: { type: 'number', description: 'Maximum price (currency inferred from deliver_to, default SGD)' },
        limit: { type: 'integer', description: 'Number of results (max 100, default 20)', default: 20 },
        offset: { type: 'integer', description: 'Pagination offset', default: 0 },
...
async function handleFindBestPrice(args: Record<string, unknown>) {
  ...
  const country = (((args.deliver_to as string) || (args.country_code as string) || (args.country as string)) || 'SG').toUpperCase();
  const region = (args.region as string) || '';
  const category = (args.category as string) || '';
  const limit = 10;
  ...
  return {
    ...
    meta: { total: data.length, country, response_time_ms: Date.now() - t0, ...(shoppingJobId ? { shopping_job_id: shoppingJobId } : {}) },
  };
}
...
const VALID_COUNTRY_CODES: Record<string, string[]> = {
  ...
  search_products_v2: ['SG', 'US', 'VN', 'TH', 'MY'],
  get_deals: ['SG', 'US', 'VN', 'TH', 'MY'],
  get_deals_v2: ['SG', 'US', 'VN', 'TH', 'MY'],
  list_categories: ['SG', 'US', 'VN', 'TH', 'MY', 'GB', 'IN', 'AU'],
  find_best_price: ['SG', 'MY', 'TH', 'PH', 'VN', 'ID', 'US'],
  find_best_price_v2: ['SG', 'MY', 'TH', 'PH', 'VN', 'ID', 'US'],
};
  ...
  try {
    switch (method) {
      case 'tools/call': {
        ...
        // (BUY-72481 owns the 2026-12-31 sunset).
