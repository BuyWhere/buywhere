...
// Null when neither vector DB env var is set; consumers must check before using.
// BUY-41137: tolerate the live Railway alias (`VECTOR_DB_URL`) and the older
// deployment name (`VECTOR_STORE_DATABASE_URL`) during the rollout.
const VECTOR_DB_CONNECTION_STRING =
  process.env.VECTOR_DB_URL || process.env.VECTOR_STORE_DATABASE_URL || '';

export const vectorDb: Pool | null = VECTOR_DB_CONNECTION_STRING
  ? new Pool({
      connectionString: VECTOR_DB_CONNECTION_STRING,
      max: 5,
      idleTimeoutMillis: 30000,
