...
exports.extractProductIds = extractProductIds;
exports.hasOutboundUrl = hasOutboundUrl;
exports.startShoppingJobFunnel = startShoppingJobFunnel;
exports.stopShoppingJobFunnel = stopShoppingJobFunnel;
exports.resolveShoppingJobId = resolveShoppingJobId;
exports.recordJobCreated = recordJobCreated;
exports.recordProductResolved = recordProductResolved;
exports.recordExecutableOfferFound = recordExecutableOfferFound;
exports.recordOutboundLinkReturned = recordOutboundLinkReturned;
 ...
 */
function classifyIsInternal(apiKey) {
    if (!apiKey)
        return false;
...
function hashApiKey(rawKey) {
    if (!rawKey)
        return null;
    return (0, crypto_1.createHash)('sha256').update(rawKey).digest('hex').slice(0, 16);
}
/**
 * Derive deliver_to from tool args: deliver_to > country_code > country
 */
function deriveDeliverTo(args) {
    if (!args)
        return '';
 ...
 */
function deriveCountryCode(args) {
 ...
 */
function hasOutboundUrl(result) {
    if (!result || typeof result !== 'object')
        return false;
    const r = result;
    // Direct outbound_url field (v2 contract)
    if (r.outbound_url && typeof r.outbound_url === 'string')
        return true;
    ...
    for (const p of products) {
        if (p && typeof p === 'object') {
            const prod = p;
            if (prod.outbound_url && typeof prod.outbound_url === 'string')
                return true;
            // Also check click_url / affiliate_url for completeness
            if (prod.click_url && typeof prod.click_url === 'string')
                return true;
        }
    }
    ...
    const bestPrice = r.best_price;
    if (bestPrice) {
        if (bestPrice.outbound_url || bestPrice.url || bestPrice.click_url)
            return true;
        const alts = r.alternatives;
        if (Array.isArray(alts)) {
            for (const alt of alts) {
                if (alt?.outbound_url || alt?.url || alt?.click_url)
                    return true;
            }
        }
    }
 ...
 */
function extractMerchantInfo(result) {
    if (!result || typeof result !== 'object')
...
async function flushOnce(pool) {
    ...
    try {
        ...
        for (let i = 0; i < batch.length; i++) {
            ...
            params.push(r.shopping_job_id, r.funnel_stage, r.tool_name, r.api_key_hash, r.deliver_to, r.country_code, r.query_text, r.product_id, r.merchant_id, r.merchant_name, r.offer_url_present, r.is_internal, r.is_replay, r.stage_data ? JSON.stringify(r.stage_data) : null, r.created_at);
        }
...
// Public API
// ─────────────────────────────────────────────────────────────────────────────
function startShoppingJobFunnel(pool = config_1.catalogDb) {
    if (started)
        return;
    started = true;
    scheduleFlush(pool);
}
async function stopShoppingJobFunnel(pool = config_1.catalogDb) {
    if (flushTimer) {
 ...
 */
function resolveShoppingJobId(clientJobId, args) {
    ...
    if (productName && deliverTo) {
        ...
        try {
            const hash = (0, crypto_1.createHash)('sha1').update(sessionKey).digest();
 ...
 */
function recordProductResolved(input) {
    const deliverTo = deriveDeliverTo(input.args);
    const isInternal = classifyIsInternal(input.apiKey);
 ...
 */
function recordExecutableOfferFound(input) {
    const deliverTo = deriveDeliverTo(input.args);
    const isInternal = classifyIsInternal(input.apiKey);
    const productIds = extractProductIds(input.result);
    const primaryProductId = productIds[0] ?? null;
 ...
 */
function recordOutboundLinkReturned(input) {
    const deliverTo = deriveDeliverTo(input.args);
    const isInternal = classifyIsInternal(input.apiKey);
    const productIds = extractProductIds(input.result);
    const primaryProductId = productIds[0] ?? null;
...
function enqueueRow(row, pool = config_1.catalogDb) {
