...
async function queryWithWarmupBudget(client, sql, params) {
    try {
        ...
        if (isTimeoutError(err)) {
            console.warn('[mcp-warmup] skipped slow/locked warmup query:', err.message);
            return null;
        }
        throw err;
    }
...
async function warmupMcpCaches() {
    ...
    try {
        ...
        if (!MCP_ENABLE_STARTUP_DDL) {
           ...
           LIMIT 100`, [country]);
                ...
                await config_1.redis.set(cacheKey, JSON.stringify(data), 'EX', 600).catch(() => { });
            }
            return;
        }
    ...
    `);
        const viewCheck = await client.query(`SELECT to_regclass('public.mcp_category_summary') AS summary,
...
async function refreshCategorySummaries() {
    ...
    try {
        ...
        if (!summaryRefresh || !countrySummaryRefresh)
            return;
        for (const country of CATEGORY_REFRESH_COUNTRIES) {
            const t0 = Date.now();
            const result = await client.query(`SELECT slug, name, product_count
