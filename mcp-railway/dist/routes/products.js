...
router.get('/search', agentDetect_1.agentDetectMiddleware, apiKey_1.requireApiKey, apiKey_1.checkRateLimit, (0, queryLog_1.queryLogMiddleware)('products.search'), asyncHandler(async (req, res) => {
    ...
    const semQNorm = q ? (0, semanticCache_1.normalizeQuery)(q) : '';
    let semVec = null;
    if ((0, semanticCache_1.semanticEnabled)() && semQNorm && offset === 0) {
        try {
            const gk = process.env.GEMINI_API_KEY ?? '';
            if (gk)
                semVec = await getCachedQueryEmbedding(q, gk);
            const semHit = await (0, semanticCache_1.semanticLookup)(config_1.redis, semScope, semQNorm, semVec);
            if (semHit) {
    ...
    const responseBody = (0, response_1.buildSearchResponse)(filteredProducts, total, limit, offset, responseTimeMs, false, hasMore ?? false);
    ...
    if ((0, semanticCache_1.semanticEnabled)() && semQNorm && offset === 0) {
        (0, semanticCache_1.semanticRegister)(config_1.redis, semScope, semQNorm, semVec, cacheKey).catch(() => { });
    }
...
router.get('/deals', agentDetect_1.agentDetectMiddleware, apiKey_1.requireApiKey, apiKey_1.checkRateLimit, (0, queryLog_1.queryLogMiddleware)('products.deals'), asyncHandler(async (req, res) => {
    ...
    config_1.redis.set(cacheKey, JSON.stringify(responseBody), 'EX', deals.length === 0 ? 60 : SEARCH_CACHE_TTL_SECONDS).catch(() => { });
...
async function warmSearchCache() {
    ...
    for (const { q, country } of WARM_SEED_QUERIES) {
      ...
      `;
            ...
            const responseBody = (0, response_1.buildSearchResponse)(products, total, limit, offset, 0, false, hasMore);
