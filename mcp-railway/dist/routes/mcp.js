...
async function getRegionalProductSample(country, fallbackQuery, limit, currency, t0) {
    try {
        ...
        try {
         ...
         LIMIT $3`, [country, fallbackQuery, limit]);
            if (!result.rows.length)
                return null;
...
async function handleGetDeals(args) {
    ...
    try {
        ...
        const filterConditions = conditions.map((condition) => condition.replace(/\$(\d+)/g, (_, n) => `$${Number(n) + 1}`)).join(' AND ');
        ...
        if ((products.length === 0 || discountScanTimedOut) && country) {
            ...
            if (fallbackSample && fallbackSample.data && fallbackSample.data.length > 0) {
                const fbProducts = fallbackSample.data;
