        ...
        if (q) {
            ...
            if (useVector) {
               ...
               LIMIT ${pageLimit}`, [...candIds, q], `fts_detfb${candIds.length}`);
                ...
                const cand = await spQuery(`SELECT sp.id FROM ${ftsTable} sp ${tierWhere} LIMIT ${pageLimit}`, tierParams, `fts_id${tierParams.length}`);
                const candIds = cand.rows.map(r => r.id);
                if (candIds.length === 0) {
                    rows = [];
                    total = 0;
             ...
             WHERE id IN (${ph})
             ORDER BY rank DESC
             LIMIT ${pageLimit}`, [...candIds, q], `fts_det${candIds.length}`);
                    rows = detailResult.rows.slice(offset, offset + limit);
                    total = candIds.length + offset;
                }
            }
