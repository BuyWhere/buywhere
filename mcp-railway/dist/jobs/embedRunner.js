    ...
    await client.query(`INSERT INTO embed_watermark (partition_name, last_updated_at, rows_embedded, zero_ticks, updated_at)
     ...
     ON CONFLICT (partition_name) DO UPDATE
       SET last_updated_at = EXCLUDED.last_updated_at,
           ...
           END,
           updated_at = NOW()`, [partitionName, nextWatermark, rowsEmbedded]);
}
...
async function tick() {
    ...
    try {
        ...
        if (!partition) {
            ...
            let watermark = wm?.last_updated_at ?? undefined;
            if (watermark) {
            ...
            console.log(`[embed-runner] Sweeping partition=${partition} watermark=${watermark ? watermark.toISOString() : 'NULL'}`);
            const summary = await (0, embedProducts_1.runEmbedBatch)(liveReplicaDb, liveVectorDb, GEMINI_API_KEY, BATCH_LIMIT, partition, watermark);
...
async function main() {
    console.log(`[embed-runner] Starting (BUY-76503 partition sweep) — ` +
