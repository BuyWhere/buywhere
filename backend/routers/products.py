...
from backend.affiliate_links import get_affiliate_url, get_underlying_affiliate_url, is_valid_url
from backend.auth import get_current_api_key
from backend.database import get_db
from backend.models.product import ApiKey, Click, Product, PriceHistory, ProductView, ProductMatch, ProductReview, ProductQuestion, ProductAnswer
from backend.rate_limit import limiter, rate_limit_from_request
...
def _build_json_ld(p: Product, target_currency: Optional[str] = None) -> str:
    ...
    if target_currency and target_currency != p.currency:
        try:
            from backend.services.currency import convert_price as _convert
...
from backend.schemas.product import CompareResponse
from backend.schemas.product import CompareMatch
from backend.schemas.product import CompareHighlights
from app import cache
from backend.currency import convert_price, build_currency_headers, SUPPORTED_CURRENCIES, get_exchange_rate
from backend.routers.deals import DealItem, DealsResponse as DealsResponseBase
...
def _map_product(p: Product, price_trend: Optional[str] = None, target_currency: Optional[str] = None, confidence_score: Optional[float] = None) -> ProductResponse:
    ...
    return ProductResponse(
        ...
        region=p.region if p.region is not None else "sg",
        country_code=p.country_code if p.country_code is not None else "SG",
...
async def compare_product_search(
    ...
    from backend.compare import ProductMatcher
...
async def get_product_deals(
    ...
    from backend.routers.deals import _to_deal_item
...
async def get_product_matches(
    ...
    from backend.services.matches import MatchService
...
async def get_also_searched(
    ...
    from backend.recommendations_engine import CoOccurrenceEngine
