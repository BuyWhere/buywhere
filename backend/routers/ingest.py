...
from backend.models.product import ApiKey, Product, IngestionRun, PriceHistory
...
from backend.services.product_quality import compute_ingest_quality_score, TIER_CRITICAL_FAIL_LABEL
    ...
    if rows_inserted > 0 or rows_updated > 0:
        ...
        for event in webhook_events:
            if event["old_price"] is not None or not event["is_new"]:
                from backend.services.price_alert import check_and_trigger_price_alerts
                try:
