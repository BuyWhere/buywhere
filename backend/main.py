...
from backend.auth import ApiKeyContextMiddleware
from backend.rate_limit import limiter, TierRateLimitMiddleware, RedisPerMinuteRateLimitMiddleware
from backend.request_logging import RequestLoggingMiddleware
from backend.usage_metering import UsageMeteringMiddleware
from backend.routers import products, categories, keys, deals, ingestion, ingest, search, status, catalog, agents, analytics, admin, developers, webhooks, metrics, alerts, images, changelog, feed, merchants, trending, export, enrichment, health, brands, watchlist, dedup, compare, billing, countries, sitemap, v2, merchant_analytics, affiliate, preferences, import_csv, saved_searches, usage, referrals, coupons, linkless_attribution, scraper_assignments, scraper_alerts, scraper_refresh, agent_native, newsletter, user_watchlist, user_alerts, users, referral_landing, push_notifications, user_notification_preferences, price_drops, growth, feature_flags, signup, stats, public_alerts, alertmanager_webhooks, auth_compat, demo, queries, mcp, monitoring
from app import clickthrough
from backend.graphql import graphql_router
from backend.versioning import VersionRoutingMiddleware
from backend.services.health import get_db_health, check_disk_space, check_api_self_test
from backend.services.scraper_health import get_scraper_health
from backend.schemas.status import ComprehensiveHealthReport, DiskSpaceHealth, APIResponseTimeHealth, DBHealthReport, ScraperHealthReport
from backend.sentry import init_sentry, is_sentry_enabled, capture_exception_with_context, SentrySlowQueryMiddleware
from backend.logging_centralized import get_logger
...
async def lifespan(app: FastAPI):
    ...
    init_sentry()
    try:
        from backend.services.feature_flags_configmap import get_configmap_syncer
        get_configmap_syncer()
    except Exception:
        pass
    from backend.services.disk_watchdog import start_disk_watchdog, stop_disk_watchdog
    ...
    try:
        from backend.services.feature_flags_configmap import stop_configmap_syncer
...
from backend.routers.admin_comparison_pages import router as admin_comparison_pages_router
...
async def health_db_root():
    ...
    from backend.routers.health import health_db
...
async def health_redis_root():
    from backend.routers.health import health_redis
...
async def openai_tools_schema():
    from backend.schemas.tools import OPENAI_TOOLS
    return JSONResponse(
...
async def mcp_tools_schema():
    from backend.schemas.tools import MCP_TOOLS
    return JSONResponse(
