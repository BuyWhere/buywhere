...
//
// BUY-60378: set statement_timeout on replicaDb so the embed-runner's catalog
// SELECT fails deterministically (57014) instead of waiting for the Railway
// proxy idle-client teardown (~3 min). 60 s gives the index-backed scan ample
// room while capping wasted wall-clock on a planner regression.
