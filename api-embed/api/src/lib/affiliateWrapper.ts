...
export interface AffiliateConfig {
  platform: string;      // e.g. 'shopee_sg', 'lazada_sg'
  networkId: string;     // affiliate network name, e.g. 'accesstrade', 'involve_asia'
  trackingId: string;    // your publisher/tracking ID on the network
  isActive: boolean;
}
...
let configCache: Map<string, AffiliateConfig> = new Map();
let cacheLoadedAt = 0;
...
export async function loadAffiliateConfigs(): Promise<void> {
  try {
    ...
    const next = new Map<string, AffiliateConfig>();
...
async function getConfig(platform: string): Promise<AffiliateConfig | null> {
 ...
 */
export async function wrapAffiliateUrl(
  rawUrl: string,
  productId: string,
  merchantId: string | null,
): Promise<{ url: string; clickId: string | null }> {
 ...
 */
export function resolvePrecomputedAffiliateUrl(affiliateUrl: unknown): string | null {
