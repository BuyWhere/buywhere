"""
Home Depot US sitemap-based scraper - proxy-free approach
Target: 30K+ products using direct sitemap XML crawling
"""
...
class HomeDepotSitemapScraper:
    ...
    async def extract_product_urls_from_sitemap(self, sitemap_url: str) -> List[str]:
        ...
        try:
            ...
            print(f"Extracted {len(product_urls)} product URLs from {sitemap_url}")
            return product_urls
    ...
    async def run(self) -> dict[str, Any]:
        ...
        print(f"Starting Home Depot sitemap scraper with limit {self.limit}")
        ...
        if not sitemap_urls:
            print("No sitemaps found, cannot proceed")
            return {
                'status': 'error',
        ...
        print(f"Found sitemaps: {sitemap_urls}")
        
        # Step 2: Extract product URLs from sitemaps
        print("Step 2: Extracting product URLs from sitemaps...")
        all_product_urls = []
        
        for sitemap_url in sitemap_urls:
            product_urls = await self.extract_product_urls_from_sitemap(sitemap_url)
            all_product_urls.extend(product_urls)
        
        # Remove duplicates and filter
        unique_urls = list(set([url for url in all_product_urls if self._is_product_url(url)]))
        print(f"Total unique product URLs found: {len(unique_urls)}")
        
        if len(unique_urls) == 0:
            print("No product URLs found in sitemaps")
            return {
                'status': 'error',
        ...
        urls_to_scrape = unique_urls[:self.limit]
        print(f"Will scrape {len(urls_to_scrape)} products")
        
        # Step 3: Scrape products
        print("Step 3: Scraping products...")
        products = await self.scrape_products(urls_to_scrape)
        ...
        result = {
            ...
            'products_skipped': len(urls_to_scrape) - len(products),
