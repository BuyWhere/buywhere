# BUY-53318 safe-data-cleanup sweep

Timestamp: 2026-06-19T03:24:24Z to 2026-06-19T03:25:38Z
...
- No workspace crossed the routine `>= 1.00 GB` apply threshold, so no `--apply` run was permitted this heartbeat.
...
- `/dev/vda1` slipped below the preferred `25 GB` safety margin but stayed above incident territory at the end of the sweep: `22G` free (`22,859,632` 1K blocks available).

## Disk state

```text
Filesystem      Size  Used Avail Use% Mounted on
/dev/vda1       193G  171G   22G  89% /

Filesystem     1K-blocks      Used Available Use% Mounted on
/dev/vda1      202051056 179175040  22859632  89% /
```
...
- Several standard cleanup scripts still returned exit code `1` while emitting a normal summary footer. This sweep treated the parseable footer as the source of truth, consistent with the earlier routine reports.
- Because no apply run happened, there was no new trash payload, no tarball step, and no additional reclaimed space beyond the dry-run estimates.
- Parent issue [BUY-33215] should receive the current free-space reading after this sweep: `22,859,632` 1K blocks available on `/dev/vda1`.
- Free space remains above the runbook's incident threshold (`20 GB`), so no new critical `[INCIDENT] workspace disk reclaim needed` issue was opened.
