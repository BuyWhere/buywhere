# BUY-53718 safe-data-cleanup sweep

Timestamp: 2026-06-19T17:20:49Z
Date: 2026-06-19
...
- Ran `timeout 150s ./safe-data-cleanup.sh --dry-run --skip-r2 --skip-catalog-check --grace=0` in all 13 candidate workspaces and captured issue-local logs under `reports/BUY-53718-*-dryrun.log`.
...
- `/dev/vda1` ended the sweep below the preferred `25 GB` safety margin but above the `20 GB` incident threshold: `24G` free (`24,933,784` 1K blocks / `25,532,194,816` bytes available).
...
| `5bc984ee-e2d2-4312-9e6c-b2864524a21f` | 200 | 0.30 GB |
...
- Total estimated reclaim across all workspaces: 2.52 GB
...
/dev/vda1      202051056 177100888  24933784  88% /
```

## Notes

- The largest candidate set stayed below the mutation gate: workspace `a29ac9dc-cf0a-455b-964c-e75bd2f5fc47` at `0.94 GB`.
- Workspace `5bc984ee-e2d2-4312-9e6c-b2864524a21f` still hit the script's `MAX_FILES` cap during dry-run, but the capped estimate rose only to `0.30 GB`, so it remained safely below the apply threshold.
- The host remains below the routine's preferred `25 GB` safety margin, but this heartbeat had no workspace candidate at or above the contract's `1.00 GB` apply threshold to safely reclaim more space.
