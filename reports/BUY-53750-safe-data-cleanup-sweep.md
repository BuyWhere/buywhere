# BUY-53750 safe-data-cleanup sweep
...
- Ran `timeout 180s ./safe-data-cleanup.sh --dry-run --skip-r2 --skip-catalog-check --grace=0` in all 13 candidate workspaces and captured issue-local logs under `reports/BUY-53750-*-dryrun.log`.
...
- `/dev/vda1` stayed above the preferred `25 GB` safety margin at the end of the sweep: `27G` free (`28,026,092` 1K blocks available / about `28.7 GB`).
...
| `5bc984ee-e2d2-4312-9e6c-b2864524a21f` | 200 | 0.37 GB |
...
- Total estimated reclaim across all workspaces: 2.59 GB
...
/dev/vda1      202051056 174008580  28026092  87% /
```

## Notes

- The largest candidate set stayed below the mutation gate: workspace `a29ac9dc-cf0a-455b-964c-e75bd2f5fc47` at `0.94 GB`.
- Workspace `5bc984ee-e2d2-4312-9e6c-b2864524a21f` still hit the script's `MAX_FILES` cap during dry-run, but the capped estimate rose only to `0.37 GB`, so it remained below the apply threshold.
