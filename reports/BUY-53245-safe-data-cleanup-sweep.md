# BUY-53245 safe-data-cleanup sweep

Timestamp: 2026-06-19T01:07:12Z to 2026-06-19T01:08:34Z
...
- The largest standard candidate was `5bc984ee-e2d2-4312-9e6c-b2864524a21f` at `0.81 GB`, so no `safe-data-cleanup.sh --apply --skip-r2 --skip-catalog-check --grace=0 --max-files=300` run was permitted.
...
/dev/vda1      202051056 177920076  24114596  89% /
```
...
5bc984ee-e2d2-4312-9e6c-b2864524a21f  standard   status=0    --- files=182 freed=0.81GB catalogChecked=0 lowMatchKept=0 r2Missed=21 apply=0
...
- `/dev/vda1` stayed above the issue's incident threshold because free space remained `24114596 KB` (`23G`), so no new critical `[INCIDENT] workspace disk reclaim needed` issue was opened.
...
- As in prior sweeps, several zero-candidate workspaces still returned `status=1`; the operational decision was based on the summary line rather than exit code alone.
