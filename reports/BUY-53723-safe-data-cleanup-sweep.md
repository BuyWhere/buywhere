# BUY-53723 safe-data-cleanup sweep

Timestamp: 2026-06-19T17:37:42Z
Date: 2026-06-19
...
- Ran `timeout 180s ./safe-data-cleanup.sh --dry-run --skip-r2 --skip-catalog-check --grace=0` in all 13 candidate workspaces and captured issue-local logs under `reports/BUY-53723-*-dryrun.log`.
...
- `/dev/vda1` ended the sweep below the preferred `25 GB` safety margin but above the `20 GB` incident threshold: `24G` free (`24,650,744` 1K blocks / about `25.2 GB` available).
...
| `5bc984ee-e2d2-4312-9e6c-b2864524a21f` | 200 | 0.33 GB |
...
- Total estimated reclaim across all workspaces: 2.55 GB
...
/dev/vda1        202051056 177383928  24650744      88% /
```

## Notes

- The largest candidate set stayed below the mutation gate: workspace `a29ac9dc-cf0a-455b-964c-e75bd2f5fc47` at `0.94 GB`.
- Workspace `5bc984ee-e2d2-4312-9e6c-b2864524a21f` still hit the script's `MAX_FILES` cap during dry-run, but the capped estimate rose only to `0.33 GB`, so it remained below the apply threshold.
- This heartbeat intentionally left all workspaces unchanged because the sweep never crossed the routine apply threshold.
- [BUY-33215] should receive the current free-space reading after this sweep: `24,650,744` 1K blocks available on `/dev/vda1`.
