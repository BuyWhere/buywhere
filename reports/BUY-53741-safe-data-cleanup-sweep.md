# BUY-53741 safe-data-cleanup sweep

Timestamp: 2026-06-19T18:08:33Z
Date: 2026-06-19
...
- Ran `timeout 180s ./safe-data-cleanup.sh --dry-run --skip-r2 --skip-catalog-check --grace=0` in all 13 candidate workspaces and captured issue-local logs under `reports/BUY-53741-*-dryrun.log`.
- Parsed the current dry-run footer format (`--- files=... freed=...GB ...`) into `reports/BUY-53741-dryrun-summary.tsv` after the first narrow parser missed the log schema.
...
- `/dev/vda1` ended the sweep above the preferred `25 GB` safety margin and above the `20 GB` incident threshold: `27G` free (`28,128,452` 1K blocks available).
...
| `5bc984ee-e2d2-4312-9e6c-b2864524a21f` | 200 | 0.34 GB |
...
- Total estimated reclaim across all workspaces: 2.56 GB
...
/dev/vda1        202051056 173906220  28128452      87% /
```

## Notes

- The largest candidate set stayed below the mutation gate: workspace `a29ac9dc-cf0a-455b-964c-e75bd2f5fc47` at `0.94 GB`.
- Workspace `5bc984ee-e2d2-4312-9e6c-b2864524a21f` still hit the script's `MAX_FILES` cap during dry-run, but the capped estimate remained well below the apply threshold at `0.34 GB`.
- This heartbeat intentionally left all workspaces unchanged because the sweep never crossed the routine apply threshold.
- Durable evidence for this run lives in `reports/BUY-53741-dryrun-summary.tsv` and `reports/BUY-53741-*-dryrun.log`.
