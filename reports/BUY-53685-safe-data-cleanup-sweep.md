# BUY-53685 safe-data-cleanup sweep

Timestamp: 2026-06-19T16:20:55Z
Date: 2026-06-19
...
- Ran `timeout 150s ./safe-data-cleanup.sh --dry-run --skip-r2 --skip-catalog-check --grace=0` in all 13 candidate workspaces and captured issue-local logs under `reports/BUY-53685-*-dryrun.log`.
...
- `/dev/vda1` remained above the 25 GB safety margin after the sweep: `30G` free (`30,597,880` 1K blocks / `31,332,229,120` bytes available).
...
| `5bc984ee-e2d2-4312-9e6c-b2864524a21f` | 200 | 0.24 GB |
...
- Total dry-run candidates: 404 files
- Total estimated reclaim across all workspaces: 2.46 GB
...
- Workspace `5bc984ee-e2d2-4312-9e6c-b2864524a21f` hit the script's `MAX_FILES` cap during dry-run but still only estimated `0.24 GB`, so it remained safely below the apply threshold.
