...
# run-buy-57114-worker-wc-cycle-cleanup.sh
# BUY-57114: Worker node disk-space enforcement (WC cycle artifact cleanup)
#
...
EVIDENCE_DIR="$REPO_ROOT/BUY-57114-evidence"
...
log "=== BUY-57114 Worker node WC cycle artifact cleanup ==="
log "Keep=${KEEP_HOURS}h  alert=${ALERT_PCT}%"
log "Disk before: ${disk_pct_before}%"

if [ ! -x "$CLEANUP_SCRIPT" ]; then
  log "ERROR: Cleanup script not found at $CLEANUP_SCRIPT"
  exit 1
fi

# Step 1: Dry-run pass (default behavior scans all workspaces under WORKSPACES_ROOT)
...
log "=== BUY-57114 completed. exit=$APPLY_EXIT ==="
