...
# run-buy-57223-worker-wc-cycle-cleanup.sh
# BUY-57223: Worker node disk-space enforcement (WC cycle artifact cleanup)
#
...
EVIDENCE_DIR="$REPO_ROOT/BUY-57223-evidence"
...
log "=== BUY-57223 Worker node WC cycle artifact cleanup ==="
...
bash "$CLEANUP_SCRIPT" \
  ...
  --workspace-dir="$DATA_DIR" 2>&1 | tee "$EVIDENCE_DIR/dryrun-output.txt"
...
bash "$CLEANUP_SCRIPT" \
  ...
  --workspace-dir="$DATA_DIR" 2>&1 | tee "$EVIDENCE_DIR/apply-output.txt"
...
log "=== BUY-57223 completed. exit=$APPLY_EXIT ==="
