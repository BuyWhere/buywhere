...
async function getProductCountsBySource(client, hoursBack) {
  const result = await client.query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '${hoursBack} hours') AS recent_inserts,
      COUNT(*) FILTER (WHERE updated_at >= NOW() - INTERVAL '${hoursBack} hours'
                        AND created_at < NOW() - INTERVAL '${hoursBack} hours') AS recent_updates_only
    FROM products
