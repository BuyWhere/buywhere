#!/usr/bin/env bash
# run-buy-56158-disk-space-watchdog-cron.sh — BUY-56158 / BUY-48198
# Disk-space watchdog for /dev/vda1 (root filesystem). Warns at <20GB and
# creates a critical Paperclip incident at <5GB. Runs every 5 minutes via cron.
# Idempotent: state-file deduplication prevents incident spam (30 min dedup,
# 60 min warn).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
LOG_FILE="$REPO_ROOT/logs/buy-56158-disk-space-watchdog.log"
mkdir -p "$REPO_ROOT/logs"

# Free space in GB (round down) for /dev/vda1; fall back to / if unavailable.
...
STATE_DIR="/tmp/buy-56158-disk-space-watchdog"
mkdir -p "$STATE_DIR"
...
if [[ "$FREE_GB" -lt "$CRITICAL_GB" ]]; then
    ...
    create_incident \
        "CRITICAL: Disk space low on /dev/vda1 ($FREE_GB GB remaining)" \
        "critical" \
        "Free disk space on /dev/vda1 is critically low: **$FREE_GB GB** remaining (threshold: ${CRITICAL_GB}GB).\n\nTimestamp: $TS\n\nThis is an automated incident created by the disk-space watchdog (BUY-56158/BUY-48198)."
    exit 2
