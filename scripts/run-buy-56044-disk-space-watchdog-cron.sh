#!/usr/bin/env bash
# run-buy-56044-disk-space-watchdog-cron.sh — BUY-56044 / BUY-48198
# Canonical disk-space watchdog for the root filesystem.
# Monitors free space on / (handles /dev/vda1 or whatever mount). Warns at <20GB.
# Creates a critical Paperclip incident at <5GB. Runs every 5 minutes via cron.
# Idempotent: state-file deduplication prevents incident spam (30 min dedup, 60 min warn).
set -euo pipefail
...
STATE_DIR="/tmp/buy-56044-disk-space-watchdog"
mkdir -p "$STATE_DIR"
