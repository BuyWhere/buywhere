#!/usr/bin/env bash
# setup-buy-57336-worker-disk-enforcement.sh — BUY-57336 (idempotent)
#
...
RUNNER="$SCRIPT_DIR/run-buy-57336-worker-disk-enforcement.sh"
LOG_FILE="$REPO_ROOT/logs/buy-57336-disk-enforcement.log"
SETUP_LOG="$REPO_ROOT/logs/setup-buy-57336-disk-enforcement.log"

MARKER="disk-enforcement-cron"

NEW_CRON="*/10 * * * * cd $REPO_ROOT && bash $RUNNER >> $LOG_FILE 2>&1 # BUY-57336: Worker node disk-space enforcement -- $MARKER"

mkdir -p "$REPO_ROOT/logs"

log() { echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] $*" | tee -a "$SETUP_LOG"; }

log "Setting up BUY-57336 worker node disk-space enforcement cron"
