#!/usr/bin/env bash
# run-buy-56469-worker-node-disk-space-enforcement.sh
# BUY-56469: Worker node disk-space enforcement (WC cycle artifact cleanup)
#
# Runs wc-cycle-cleanup.sh --apply --keep=48 in the Oracle workspace (3ec8f6dd)
# to delete orphaned WC cycle ndjson files older than 48h. Alerts if disk > 90%.
...
REPORT_PATH="$REPO_ROOT/logs/buy-56469-wc-cycle-enforcement-report.json"
...
log "BUY-56469 enforcement completed. inner_exit=$inner_exit"
exit $inner_exit
