#!/usr/bin/env bash
# run-buy-57536-worker-disk-enforcement.sh — BUY-57536
# Worker node disk-space enforcement — cron wrapper.
#
# Runs the enforcement engine every 10 minutes via cron.
# Produces evidence reports to BUY-57536-evidence/.
# Logs to logs/buy-57536-disk-enforcement.log.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
ENFORCER="$REPO_ROOT/scripts/worker-node-disk-enforcement.sh"
EVIDENCE_DIR="$REPO_ROOT/BUY-57536-evidence"
...
log "BUY-57536 cron wrapper starting"
echo_it "BUY-57536: worker node disk-space enforcement starting"
...
echo_it "BUY-57536: enforcement ${STATUS_TEXT} disk=${DISK_PCT}% free=${DISK_FREE_GB}GB"
echo_it "Evidence: $EVIDENCE_REPORT"
log "BUY-57536 cron wrapper complete exit=$ENFORCER_EXIT disk=${DISK_PCT}% free=${DISK_FREE_GB}GB"
