#!/usr/bin/env bash
# run-buy-57703-worker-disk-enforcement.sh
# BUY-57703: Worker node disk-space enforcement
#
...
# Capture final disk state
FINAL_STATE="$EVIDENCE_DIR/enforcement-$(date +%Y%m%dT%H%M%S).json"
{
    echo "{"
    echo "  \"timestamp\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\","
    echo "  \"workspaces_root\": \"$WORKSPACES_ROOT\","
    echo "  \"enforce_pct\": $ENFORCE_PCT,"
    echo "  \"critical_pct\": $CRITICAL_PCT,"
...
log "Run complete. Exit code: $EXIT_CODE. Evidence: $FINAL_STATE"
exit $EXIT_CODE
