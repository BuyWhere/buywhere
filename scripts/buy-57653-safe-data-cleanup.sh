#!/usr/bin/env bash
# buy-57653-safe-data-cleanup.sh — Workspace safe-data-cleanup sweep
# Cleans up stale BUY-*-evidence directories for completed/closed issues.
...
LOG_FILE="$REPO_ROOT/logs/buy-57653-safe-data-cleanup.log"
...
while IFS= read -r dir; do
    if [[ -d "$dir" ]]; then
        ...
        if [[ "$dir" =~ BUY-[0-9]+-evidence$ ]]; then
            ...
            log "Deleted: $dir (${dir_size} bytes)"
        fi
    fi
...
echo "[$TS] Evidence cleanup: deleted $DELETED_COUNT dirs (~${DELETED_MB}MB). Disk: ${disk_before_gb}GB → ${disk_after_gb}GB free" >> "$REPO_ROOT/logs/buy-57653-disk-enforcement.log"
