#!/usr/bin/env bash
# setup-buy-57762-worker-node-artifact-cleanup.sh — BUY-57762
...
# consolidated entry for BUY-57762.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
RUNNER="$SCRIPT_DIR/run-buy-57762-worker-wc-cycle-cleanup.sh"
LOG_FILE="$REPO_ROOT/logs/buy-57762-worker-node-artifact-cleanup.log"
MARKER="BUY-57762: Worker node WC cycle artifact cleanup — every 6 hours"
...
COUNT="$(crontab -l | grep -Ec 'BUY-57762.*WC cycle' || true)"
if [[ "$COUNT" -eq 1 ]]; then
  echo "Verified: crontab contains exactly 1 WC cycle cleanup entry (BUY-57762)"
else
  echo "Error: expected 1 WC cycle cleanup entry, found $COUNT" >&2
  exit 1
fi

echo "BUY-57762 cron installed. Runner: $RUNNER"
