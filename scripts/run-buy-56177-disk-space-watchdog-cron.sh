#!/usr/bin/env bash
# run-buy-56177-disk-space-watchdog-cron.sh — BUY-56177 / BUY-48198
# Canonical disk-space watchdog for the root filesystem.
# Records snapshots to monitoring.disk_space and creates
# Paperclip incidents at critical thresholds.
# Monitors free space on /. Warns at <20GB. Creates a critical Paperclip incident
# at <5GB. Runs every 5 minutes via cron. Idempotent: state-file deduplication
# prevents incident spam (30 min dedup, 60 min warn).
set -euo pipefail
...
STATE_DIR="/tmp/buy-56177-disk-space-watchdog"
mkdir -p "$STATE_DIR"
