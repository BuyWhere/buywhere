#!/usr/bin/env bash
# run-buy-56213-worker-node-disk-space-enforcement.sh
# BUY-56213: Worker node disk-space enforcement (WC cycle artifact cleanup)
#
# Runs wc-cycle-cleanup.sh --apply --keep=48 in the buywhere-api workspace to
# delete orphaned WC cycle ndjson files older than 48h. Alerts if disk > 90%.
...
WORKSPACE_DIR="/paperclip/instances/default/workspaces/476c8023-3635-45bb-9f71-db6f4f5700e1"
REPORT_PATH="$REPO_ROOT/logs/buy-56213-wc-cycle-enforcement-report.json"
...
log "BUY-56213 enforcement completed. inner_exit=$inner_exit"
exit $inner_exit
