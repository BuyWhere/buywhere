#!/usr/bin/env bash
# run-buy-57677-worker-disk-enforcement.sh
# BUY-57677: Worker node disk-space enforcement
#
...
LOG_FILE="$REPO_ROOT/logs/buy-57677-disk-enforcement.log"

mkdir -p "$EVIDENCE_DIR" "$REPO_ROOT/logs"

log() { echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] BUY-57677: $*"; }
...
WORKSPACES_ROOT="${WORKSPACE_ROOT:-/paperclip/instances/default/workspaces}"
