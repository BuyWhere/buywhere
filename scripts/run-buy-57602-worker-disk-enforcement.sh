#!/usr/bin/env bash
# run-buy-57602-worker-disk-enforcement.sh — BUY-57602
# Worker node disk-space enforcement — cron wrapper.
#
# Runs the enforcement engine every 10 minutes via cron.
# Produces evidence reports to BUY-57602-evidence/.
# Logs to logs/buy-57602-disk-enforcement.log.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
ENFORCER="$REPO_ROOT/scripts/worker-node-disk-enforcement.sh"
EVIDENCE_DIR="$REPO_ROOT/BUY-57602-evidence"
...
log "BUY-57602 cron wrapper starting"
echo_it "BUY-57602: worker node disk-space enforcement starting"
...
log "BUY-57602 cron wrapper completed. status=$STATUS_TEXT disk_pct=$DISK_PCT free_gb=$DISK_FREE_GB"
echo_it "BUY-57602: enforcement completed. status=$STATUS_TEXT disk=$DISK_PCT% free=${DISK_FREE_GB}GB"
