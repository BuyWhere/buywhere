#!/usr/bin/env bash
# setup-buy-57568-worker-node-disk-space-enforcement.sh — BUY-57568 (idempotent)
#
...
# Uses a shared marker "disk-enforcement-buy-57568-cron" for deduplication.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

RUNNER="$SCRIPT_DIR/run-buy-57568-worker-disk-enforcement.sh"
LOG_FILE="$REPO_ROOT/logs/buy-57568-disk-enforcement.log"
SETUP_LOG="$REPO_ROOT/logs/setup-buy-57568-disk-enforcement.log"

MARKER="disk-enforcement-buy-57568-cron"

NEW_CRON="*/10 * * * * cd $REPO_ROOT && bash $RUNNER >> $LOG_FILE 2>&1 # BUY-57568: Worker node disk-space enforcement -- $MARKER"

mkdir -p "$REPO_ROOT/logs"

log() { echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] $*" | tee -a "$SETUP_LOG"; }

log "Setting up BUY-57568 worker node disk-space enforcement cron"
...
if [[ "$COUNT" -eq 1 ]]; then
  log "Verified: crontab contains exactly 1 disk-enforcement-buy-57568-cron entry"
else
  log "Error: expected 1 disk-enforcement-buy-57568-cron entry, found $COUNT" >&2
  exit 1
fi
