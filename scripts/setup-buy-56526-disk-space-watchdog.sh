#!/usr/bin/env bash
# setup-buy-56526-disk-space-watchdog.sh — BUY-56526 / BUY-48198
# Installs exactly ONE disk-space-watchdog cron entry for /dev/vda1.
# Replaces any existing disk-space-watchdog cron entries (BUY-5xxxx).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
RUNNER="$SCRIPT_DIR/run-buy-56526-disk-space-watchdog-cron.sh"
LOG_FILE="$REPO_ROOT/logs/buy-56526-disk-space-watchdog.log"
MARKER="BUY-56526: Disk space watchdog"
...
echo "Disk space watchdog cron installed for BUY-56526/BUY-48198. Runner: $RUNNER"
bash "$RUNNER"
