#!/usr/bin/env bash
# setup-buy-57667-worker-node-disk-space-enforcement.sh — BUY-57667 (idempotent)
#
...
# Uses a shared marker "disk-enforcement-buy-57667-cron" for deduplication.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

RUNNER="$SCRIPT_DIR/run-buy-57667-worker-wc-cycle-cleanup.sh"
ENFORCE_RUNNER="$SCRIPT_DIR/run-buy-57667-worker-disk-enforcement.sh"
LOG_FILE="$REPO_ROOT/logs/buy-57667-disk-enforcement.log"
SETUP_LOG="$REPO_ROOT/logs/setup-buy-57667-disk-enforcement.log"

MARKER="disk-enforcement-buy-57667-cron"
CLEANUP_MARKER="wc-cleanup-buy-57667-cron"

NEW_CRON_ENFORCE="*/10 * * * * cd $REPO_ROOT && bash $ENFORCE_RUNNER >> $LOG_FILE 2>&1 # BUY-57667: Worker node disk-space enforcement -- $MARKER"
NEW_CRON_CLEANUP="*/30 * * * * cd $REPO_ROOT && bash $RUNNER >> $LOG_FILE 2>&1 # BUY-57667: WC cycle artifact cleanup -- $CLEANUP_MARKER"

mkdir -p "$REPO_ROOT/logs"

log() { echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] $*" | tee -a "$SETUP_LOG"; }

log "Setting up BUY-57667 worker node disk-space enforcement cron"
...
log "Log: $LOG_FILE"
log ""
log "To verify: crontab -l | grep 'BUY-57667'"
