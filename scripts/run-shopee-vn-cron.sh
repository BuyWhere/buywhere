#!/bin/bash
#
# run-shopee-vn-cron.sh — BUY-40828 scrape + ingest Shopee Vietnam
#
# Runs scripts/shopee_vn_scraper.mjs in scrape-only mode, then ingests the
...
#   SCRAPERAPI_KEY           (recommended: ultra_premium for Shopee VN WAF)
#   BRIGHTDATA_SB_HOST
...
NDJSON_OUT="$REPO_ROOT/data/affiliate_ndjson/shopee_vn.ndjson"

mkdir -p "$LOG_DIR" "$REPORT_DIR" "$(dirname "$NDJSON_OUT")"

TS=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
LOG_FILE="$LOG_DIR/shopee-vn-cron-${TS}.log"
REPORT_FILE="$REPORT_DIR/shopee-vn-cron-${TS}.json"

# API base (default to production)
API_BASE="${BUYWHERE_API_URL:-https://api.buywhere.ai}"

echo "[$TS] Starting Shopee VN scrape" | tee "$LOG_FILE"

# Step 1: scrape to NDJSON
set +e
node "$SCRIPT_DIR/shopee_vn_scraper.mjs" \
...
async function main() {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const resp = await fetch(`${API_BASE}/v1/ingest/products`, {
      ...
      body: JSON.stringify({ source: 'shopee_vn', products: batch }),
    });
    if (!resp.ok) {
