#!/usr/bin/env bash
# run-buy-56286-disk-space-watchdog-cron.sh — BUY-56286 / BUY-48198
...
LOG_FILE="$REPO_ROOT/logs/buy-56286-disk-space-watchdog.log"
STATE_DIR="/tmp/buy-56286-disk-space-watchdog"
...
if [[ "$FREE_GB" -lt "$CRITICAL_GB" ]]; then
    ...
    create_incident \
        "CRITICAL: Disk space low on /dev/vda1 ($FREE_GB GB remaining) — BUY-56286/BUY-48198" \
        "critical" \
        "Free disk space on /dev/vda1 is critically low: **$FREE_GB GB** remaining (threshold: ${CRITICAL_GB}GB).\n\nTimestamp: $TS\n\nAutomated incident — disk-space watchdog (BUY-56286/BUY-48198)."
    exit 2
