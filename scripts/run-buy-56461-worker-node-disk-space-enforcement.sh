#!/usr/bin/env bash
# run-buy-56461-worker-node-disk-space-enforcement.sh
# BUY-56461: Worker node disk-space enforcement (WC cycle artifact cleanup)
#
# Runs wc-cycle-cleanup.sh --apply --keep=48 in the agent-compass workspace
# (19dcd635) to delete orphaned WC cycle ndjson files older than 48h.
# Alerts if disk > 90%. Prevents the root filesystem hitting 100% (BUY-30774).
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
WORKSPACE_DIR="/paperclip/instances/default/workspaces/19dcd635-1d2b-4e41-9950-5865876e12b2"
REPORT_PATH="$REPO_ROOT/logs/buy-56461-wc-cycle-enforcement-report.json"
LOG_PATH="$WORKSPACE_DIR/data/_wc_cleanup_log.jsonl"

RUN_TS=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

disk_used_pct() {
  df -Pk / | awk 'NR==2 {gsub("%","",$5); print $5}'
}

log() { echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] $*"; }

inner_exit=0

disk_pct_before=$(disk_used_pct)
KEEP_HOURS=48

log "Disk before: ${disk_pct_before}% keep_hours=${KEEP_HOURS}"
log "Workspace: $WORKSPACE_DIR"

bash "$SCRIPT_DIR/wc-cycle-cleanup.sh" \
  --apply \
  --keep="$KEEP_HOURS" \
  --workspace-dir="$WORKSPACE_DIR" \
  --alert-pct=90 \
  --log-path="$LOG_PATH" \
  --report-path="$REPORT_PATH" || inner_exit=$?

if [ -s "$REPORT_PATH" ]; then
  tmp_report=$(mktemp)
  cp "$REPORT_PATH" "$tmp_report"
  python3 -c "
import json, sys
with open(sys.argv[1], 'r') as f:
    r = json.load(f)
r['ts'] = sys.argv[2]
r['issue'] = 'BUY-56461'
r['keep_hours'] = int(sys.argv[3])
r['disk_before_pct'] = int(sys.argv[4])
r['run_started_at'] = sys.argv[2]
with open(sys.argv[1], 'w') as f:
    json.dump(r, f, indent=2)
" "$tmp_report" "$RUN_TS" "$KEEP_HOURS" "$disk_pct_before"
  mv "$tmp_report" "$REPORT_PATH"
fi

disk_pct_after=$(disk_used_pct)
log "Disk after: ${disk_pct_after}%"
log "Report: $REPORT_PATH"

if [ "$inner_exit" -ne 0 ] && [ "$inner_exit" -ne 10 ]; then
  log "WARNING: wc-cycle-cleanup.sh exited with $inner_exit"
  exit "$inner_exit"
fi

log "BUY-56461 enforcement completed. inner_exit=$inner_exit"
exit $inner_exit
