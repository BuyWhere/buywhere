...
/** Primary SKUs first, accessories after — first 8 stay primary when ≥8 exist. */
export function rankPrimaryProductsFirst(products: LandingProduct[]): LandingProduct[] {
  const primary = products.filter((p) => !isGenericAccessoryProduct(p));
  const accessories = products.filter((p) => isGenericAccessoryProduct(p));
  const floorSource = primary.length > 0 ? primary : accessories;
  const floorId = findFloorPriceProductId(floorSource);
  const sortGroup = (group: LandingProduct[]) =>
    [...group].sort((a, b) => compareLandingCardOrder(a, b, undefined, floorId));
  return [...sortGroup(primary), ...sortGroup(accessories)];
}
