...
import { HOTLINK_BLOCKED_HOSTS, REFERER_GATED_HOSTS, viaImageProxy } from "@/lib/hotlink-hosts";
...
function isUsableProductImage(imageUrl?: string | null) {
  ...
  try {
    ...
    if (HOTLINK_BLOCKED_HOSTS.has(url.hostname) || REFERER_GATED_HOSTS.has(url.hostname)) return true;
