...
// BUY-72180: /products/{1-7 digit numeric} hard-404 gate.
  ...
  // BUY-81836: legacy /product/{slug} redirect to canonical /products/us/{slug}.
  // The old singular format was used in early builds but is no longer supported.
  // Redirect to the US products canonical path.
  const legacyProductMatch = /^\/product\/([^/]+)\/?$/.exec(pathname);
  if (legacyProductMatch) {
    const slug = legacyProductMatch[1];
    const url = request.nextUrl.clone();
    url.pathname = `/products/us/${slug}`;
    return tagAgent(NextResponse.redirect(url, 308));
  }

  // BUY-81836: root-level product slugs (e.g. /HP-Victus-15-FA2728TX-Gaming-Laptop--Mica-Silver...)
  // These were generated by old builds but the route no longer exists. Redirect to /products/us/{slug}.
  // Pattern: starts with letter, contains alphanumeric+hyphens, no slashes.
  const rootProductSlugMatch = /^\/([a-zA-Z][a-zA-Z0-9\-]{10,})\/?$/.exec(pathname);
  if (rootProductSlugMatch) {
    const slug = rootProductSlugMatch[1];
    // Only redirect if it looks like a product slug (contains at least one digit for product ID)
    if (/\d/.test(slug)) {
      const url = request.nextUrl.clone();
      url.pathname = `/products/us/${slug}`;
      return tagAgent(NextResponse.redirect(url, 308));
