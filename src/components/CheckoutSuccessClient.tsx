      ...
      <div className="mt-8 rounded-3xl border border-slate-200 bg-slate-50 p-6">
        {loading ? (
          ...
          <div className="space-y-3">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">Current plan</p>
            ...
            {sessionId && <p className="text-xs text-slate-500">Stripe session: {sessionId}</p>}
          </div>
        ) : (
          <p className="text-sm text-slate-600">Subscription details are not available yet. Open the dashboard to refresh the account state.</p>
        )}
      </div>
