...
export function ProductGridCard({ product, compact = false }: { product: LandingProduct; compact?: boolean }) {
  ...
  // fallback products now point every card anchor — image, merchant tag,
  // title, and the would-be "Buy at" CTA — at the same canonical PDP. This
  // produces parity with US-market pages, where live cards use /r/direct and
  // fallbacks now consistently use /products/{region}/… instead of leaking
  // /search?q= hrefs into SSR HTML. `isMerchantOffer` still evaluates to
  // false for fallbacks (productUrl starts with /products/, not /r/ or
  // http://), so the CTA renders the passive "Compare prices" span and the
  // "View details" Next.js Link uses the same PDP — no fake merchant button
  // is shown, and there is no /search?q= loopback anywhere on the card.
