...
type Frontmatter = {
  ...
  category?: string; tags?: string[]; published?: string;
};
...
export async function generateStaticParams() {
  return getAll().map((d) => ({ slug: [d!.slug] })).filter(Boolean);
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: toSiteUrl(`/directory/${doc.slug}`) },
    openGraph: { title: doc.title, description: doc.description, type: "website", url: toSiteUrl(`/directory/${doc.slug}`), siteName: "BuyWhere" },
  };
}

function buildSchema(doc: NonNullable<ReturnType<typeof getBySlug>>) {
  return {
    "@context": "https://schema.org",
    "@type": "SoftwareApplication",
    "@id": `https://buywhere.ai/directory/${doc.slug}#software`,
    name: doc.title, description: doc.description,
    applicationCategory: "DeveloperApplication",
    operatingSystem: "Any (MCP-compatible client)",
    url: "https://buywhere.ai",
    sameAs: ["https://github.com/BuyWhere/buywhere-mcp", "https://www.npmjs.com/package/@buywhere/mcp-server", "https://smithery.ai/server/buywhere"],
    offers: { "@type": "Offer", price: "0", priceCurrency: "USD", description: "Free tier: 1,000 API calls/month" },
    publisher: { "@type": "Organization", "@id": "https://buywhere.ai/#organization", name: "BuyWhere", url: "https://buywhere.ai" },
  };
}

export default function DirectoryContentPage({ params }: Params) {
  const doc = getBySlug(params.slug);
  if (!doc) notFound();
  ...
  let body = "";
  try {
    const source = fs.readFileSync(path.join(contentDir, `${doc.slug}.md`), "utf8");
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50">
      <Script id="schema" type="application/ld+json" strategy="afterInteractive">{JSON.stringify(schema)}</Script>
      <Nav />
      <main id="main-content" className="flex-1">
        <section className="border-b border-slate-200 bg-white">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 md:py-16">
            <Link href="/directory" className="mb-6 inline-flex text-sm font-medium text-indigo-600">← Browse all directories</Link>
            ...
            <div className="mb-4 flex flex-wrap gap-2 text-sm text-slate-500">
              ...
              {doc.tags.slice(0, 5).map((tag) => <span key={tag} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">{tag}</span>)}
            </div>
