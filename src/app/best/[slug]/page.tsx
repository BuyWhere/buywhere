...
function buildItemListSchema(products: Awaited<ReturnType<typeof fetchProductsForSlug>>['products'], slug: string, country: string) {
  const base = toSiteUrl(`/best/${slug}`);
  return {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `Best products for "${slug.replace(/-/g, ' ')}" in ${country}`,
    url: base,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Best ${queryPart}${displayCountry ? ` in ${displayCountry}` : ''} — BuyWhere`,
    description: `Discover the best ${queryPart}${displayCountry ? ` available in ${displayCountry}` : ''}. Compare prices across top merchants and find your ideal product at the best price.`,
    alternates: { canonical: toSiteUrl(`/best/${slug}`) },
  };
}
...
export default async function BestPage({ params }: Props) {
  ...
  const { products, query } = await fetchProductsForSlug(slug, countryCode, 'best');
      ...
      <main id="main-content" className="flex-1">
        ...
        <section className="border-b border-slate-200 bg-slate-50">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
            <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-2 text-sm text-slate-500">
              ...
              <span>/</span>
              <Link href="/best" className="hover:underline">Best Products</Link>
              <span>/</span>
              <span className="text-slate-700 capitalize">{queryFormatted}</span>
            </nav>
            <h1 className="mb-4 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">
              Best {queryFormatted}{displayCountry ? ` in ${displayCountry}` : ''}
            </h1>
            <p className="text-lg text-slate-600">
              Compare prices from top merchants and find the best {queryFormatted} for your needs.
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
          <div className="mb-8 flex items-center justify-between">
            <h2 className="text-2xl font-semibold text-slate-900">Top Products</h2>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {products.map((product) => (
              <a
                ...
                className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-amber-300 hover:shadow-lg"
              >
