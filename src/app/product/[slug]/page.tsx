...
// Derive a readable product name from the URL slug for display and social metadata.
// This enables product-specific metadata even when the catalog API is unavailable.
function deriveProductNameFromSlug(slug: string): string {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // When catalog is unavailable or product not found, still emit product-specific
  // metadata derived from the URL slug instead of homepage fallback.
  const productName = resolvedProduct
    ? resolvedProduct.name
    : deriveProductNameFromSlug(params.slug);

  const canonicalUrl = `${INTERNAL_ORIGIN}/product/${params.slug}`;
  const socialImage = `/api/og-image?title=${encodeURIComponent(productName)}`;

  return {
    title: `${productName} — BuyWhere`,
    description: `Compare prices for ${productName} across US retailers. Find the best deals on BuyWhere.`,
    alternates: {
      canonical: canonicalUrl,
    },
    openGraph: {
      title: `${productName} — BuyWhere`,
      description: `Compare prices for ${productName} across US retailers.`,
      url: canonicalUrl,
      images: [
        {
          url: socialImage,
          width: 1200,
          height: 630,
          alt: `${productName} — Compare prices on BuyWhere`,
        },
      ],
    },
    twitter: {
...
export default async function ProductDetailPage({ params }: PageProps) {
  ...
  // When catalog is unavailable or product not found, derive product name from slug
  // and render a useful page instead of redirecting. This keeps the URL stable
  // for social sharing while still offering a search link.
  const productName = resolvedProduct
    ? resolvedProduct.name
    : deriveProductNameFromSlug(params.slug);

  let initialData: USProduct | undefined;
  if (resolvedProduct) {
    initialData = await fetchUSProductSSR(resolvedProduct.id);
  }

  // Use SSR data if available, otherwise fall back to a minimal product object
  const product = initialData ?? {
    id: resolvedProduct?.id ?? params.slug,
    name: productName,
    image: "/og-image.png",
    description: `Compare current catalog offers and merchant options for ${productName} on BuyWhere.`,
    specs: { Region: "United States", "Catalog source": resolvedProduct ? "BuyWhere US product catalog" : "Catalog unavailable" },
    prices: [],
    overallRating: 0,
    reviewCount: 0,
    brand: "",
    sku: `SKU-${resolvedProduct?.id ?? params.slug}`,
          ...
          <div className="p-6">
            ...
            <div className="mt-6 pt-4 border-t border-gray-100">
              <p className="text-xs text-gray-400">
                ...
                .{" "}
                <Link
                  href={resolvedProduct ? `/products/us/${resolvedProduct.slug}` : slugToSearchRedirect(params.slug)}
