...
async function callScraper(scraperUrl: string, platform: string, skus: string[]): Promise<void> {
...
export async function runPriceRefresh(): Promise<RefreshSummary> {
  ...
  const byPlatform = new Map<string, { sku: string; product_id: string; slug: string }[]>();
  ...
  console.log(
    `[price-refresh] Done — success: ${successes.length}, failed: ${failures.length} / ${rows.length}`
  );
