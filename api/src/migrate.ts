...
export async function runMigrations() {
  ...
  try {
    ...
    const idxCheck = await db.query(
      `SELECT 1 FROM pg_indexes WHERE indexname = 'idx_products_sku_source_country'`
    );
    if (idxCheck.rows.length === 0) {
      ...
      await db.query(
        `CREATE UNIQUE INDEX CONCURRENTLY idx_products_sku_source_country ON products(sku, source, country_code)`
      );
