...
export function classifyProbeResult(input: {
  ...
  if (input.timedOut) return { status: 'transient', reason: 'timeout_gt_10s' };
  ...
  if (code === 400) return { status: 'dead', reason: 'http_400' };
  if (code === 404 || code === 410 || code === 451) return { status: 'dead', reason: `http_${code}` };
