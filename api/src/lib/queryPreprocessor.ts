...
function buildCountryPattern(): RegExp {
...
export function preprocessSearchQuery(
  ...
  const result: PreprocessedQuery = { cleanedQuery: q };
  ...
  if (maxMatch) {
    ...
    if (!isNaN(val) && val > 0) {
      if (existingMaxPrice === undefined) result.extractedMaxPrice = val;
  ...
  if (minMatch) {
    ...
    if (!isNaN(val) && val > 0) {
      if (existingMinPrice === undefined) result.extractedMinPrice = val;
