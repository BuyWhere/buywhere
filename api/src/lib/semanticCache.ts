...
function parseVector(vectorJson: string | null): number[] | null {
...
export async function semanticLookup(
  redis: any, scope: string, qNorm: string, vectorJson: string | null
): Promise<{ body: string; matchedQ: string; sim: number } | null> {
  try {
    const regKey = `semq:${scope}`;
    const reg: Record<string, string> = (await redis.hgetall(regKey)) || {};
    ...
    if (exact) {
      ...
      if (body) {
        ...
        if (!entry.v && vectorJson) {
          ...
          if (v) redis.hset(regKey, qNorm, JSON.stringify({ k: entry.k, v })).catch(() => {});
        }
    ...
    if (!vec) return null;
    let best: { q: string; k: string; sim: number } | null = null;
    for (const [entryQ, raw] of Object.entries(reg)) {
      if (entryQ === qNorm) continue;
      let entry; try { entry = JSON.parse(raw); } catch { continue; }
      if (!entry.v) continue;
      const sim = cosine(vec, entry.v);
      if (sim >= SIM_THRESHOLD && (!best || sim > best.sim)) best = { q: entryQ, k: entry.k, sim };
    }
    if (!best) return null;
    const body = await redis.get(best.k);
    if (!body) { redis.hdel(regKey, best.q).catch(() => {}); return null; }
    return { body, matchedQ: best.q, sim: best.sim };
  } catch { return null; }
}

/** Register a freshly cached response under its normalized query. Fire-and-forget. */
export async function semanticRegister(
  redis: any, scope: string, qNorm: string, vectorJson: string | null, cacheKey: string
): Promise<void> {
  try {
    const regKey = `semq:${scope}`;
    const n = await redis.hlen(regKey);
    if (n >= REGISTRY_MAX) await redis.del(regKey);
    const v = parseVector(vectorJson);
