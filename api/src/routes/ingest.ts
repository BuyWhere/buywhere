...
// BUY-79109: Keepa amazon_us products never entered search_products. After
// products upsert, promote a tiny id list (GIN idx_sp_fts has fastupdate=off
// so batches >5 stall). Fire-and-forget so ingest latency is unchanged.
const SEARCH_PRODUCTS_SINK_SOURCES = new Set(['amazon_us']);
const SEARCH_PRODUCTS_SINK_BATCH = Math.max(
  1,
  Math.min(5, parseInt(process.env.SEARCH_PRODUCTS_SINK_BATCH || '5', 10) || 5),
);

async function promoteAmazonUsToSearchProducts(productIds: Array<number | string>): Promise<void> {
...
async function handleIngest(req: Request, res: Response): Promise<void> {
    ...
    if (SEARCH_PRODUCTS_SINK_SOURCES.has(source)) {
      ...
      void promoteAmazonUsToSearchProducts(sinkIds);
    }
