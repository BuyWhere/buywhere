...
function htmlPage(title: string, description: string, jsonld: object, bodyHtml: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  if (price !== null) offerBase.price = price;
  ...
  const categoryPath: string[] = Array.isArray(p.category_path) ? p.category_path : [];
  const breadcrumb = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    '@id': `${base}/#breadcrumb`,
  ...
  const breadNav = [
    ...
    ...categoryPath.map(
      (cat: string) =>
        `<a href="/c/${encodeURIComponent(cat.toLowerCase().replace(/[^a-z0-9]+/g, '-'))}">${escHtml(cat)}</a>`
    ),
