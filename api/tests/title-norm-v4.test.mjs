...
//   - v7: hard 50-char cap for single-segment titles >50 chars that slip through
//     forward+backward trim (laptop spec-dumps, no-comma earbuds strings)
...
const cases = [
  ...
  // Backward trim finds no trailing generics. Result unchanged → 71 raw chars.
  // v7 hard cap: drop from end until prefix ≤50.
  ['HP Omnibook 5 AI Laptop 16 inch 2K WUXGA 16GB RAM 512GB SSD Win 11 Home',
    ...
    'HP Omnibook single-seg spec-dump: v7 hard cap at 50'],
  ...
  ['HP DK4Q5AT laptop Intel Core Ultra 7 33.8 cm (13.3") WUXGA 16 GB LPDDR5x-SDRAM 512 GB SSD Wi-Fi 7 (802.11be) Windows 11',
    ...
    'HP DK4Q5AT live: v7 hard cap at 50'],
  ...
  // Boundary: exactly 50 chars — should NOT be capped (v7 triggers at >50)
  ['HP Omnibook 5 AI Laptop 16 inch 2K WUXGA 16GB RAM',
    t => t.length === 49,
    'HP 49-char boundary: unchanged (v7 triggers at >50 only)'],
];
