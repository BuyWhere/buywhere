...
const liveDb = config_1.db;
let running = false;
/**
 * Load the watermark for a partition. Returns null if the partition has not
 * started yet (first boot).
 */
async function loadWatermark(sourceDb, partitionName) {
    const { rows } = await sourceDb.query(`SELECT partition_name, last_updated_at, rows_embedded, zero_ticks
     FROM embed_watermark
     WHERE partition_name = $1`, [partitionName]);
    return rows[0] ?? null;
}
/**
 * Upsert watermark after a tick. Called within the tick transaction.
 */
async function upsertWatermark(client, partitionName, nextWatermark, rowsEmbedded) {
    const zeroTicks = rowsEmbedded === 0 ? 1 : 0;
    await client.query(`INSERT INTO embed_watermark (partition_name, last_updated_at, rows_embedded, zero_ticks, updated_at)
     VALUES ($1, $2, $3, $4, NOW())
     ON CONFLICT (partition_name) DO UPDATE
       SET last_updated_at = EXCLUDED.last_updated_at,
           ...
           END,
           updated_at = NOW()`, [partitionName, nextWatermark, rowsEmbedded, zeroTicks]);
}
/**
 ...
 */
async function pickNextPartition(sourceDb, roundRobinIdx) {
    const { rows } = await sourceDb.query(`SELECT partition_name, zero_ticks FROM embed_watermark`);
...
async function tick() {
    ...
    try {
        ...
        if (!partition) {
            ...
            await liveDb.query(`UPDATE embed_watermark SET zero_ticks = 0 WHERE zero_ticks >= $1`, [STALE_SKIP_THRESHOLD]);
        }
        else {
            ...
            const watermark = wm?.last_updated_at ?? undefined;
            ...
            console.log(`[embed-runner] Sweeping partition=${partition} watermark=${effectiveWatermark ? effectiveWatermark.toISOString() : 'NULL'}`);
            // 4. Run the embed batch on this partition.
            const summary = await (0, embedProducts_1.runEmbedBatch)(liveReplicaDb, liveVectorDb, FLOWAI_KEY, BATCH_LIMIT, partition, effectiveWatermark);
            console.log(`[embed-runner] ${partition} tick complete — ` +
...
async function main() {
    console.log(`[embed-runner] Starting (BUY-76503 partition sweep, BUY-76567 Flow AI) — ` +
    ...
    try {
        await liveVectorDb.query(`ALTER TABLE product_embeddings ADD COLUMN IF NOT EXISTS embedding_v2 vector(1024)`);
    ...
    try {
      ...
      END $$;
    `);
        await liveVectorDb.query(`GRANT INSERT, UPDATE ON product_embeddings TO embed_writer`);
    ...
    try {
        await liveDb.query(`INSERT INTO embed_watermark (partition_name, last_updated_at, rows_embedded, zero_ticks)
       SELECT p, NULL, 0, 0
       FROM unnest($1::text[]) AS p
       ON CONFLICT (partition_name) DO NOTHING`, [PARTITION_ORDER]);
