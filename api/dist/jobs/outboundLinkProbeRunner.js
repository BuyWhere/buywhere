...
async function fetchDueRows() {
    ...
    const neverChecked = await config_1.db.query(`SELECT id::text, merchant_id, url
       FROM products
      WHERE is_active = true
        ...
        ${countryFilter}
      LIMIT $1`, params);
    if (neverChecked.rows.length >= BATCH_SIZE)
        return neverChecked.rows;
    const staleParams = [BATCH_SIZE - neverChecked.rows.length];
    let staleCountryFilter = '';
    if (COUNTRY) {
        staleParams.push(COUNTRY);
        staleCountryFilter = `AND country_code = $${staleParams.length}`;
    }
    const stale = await config_1.db.query(`SELECT id::text, merchant_id, url
       FROM products
      WHERE is_active = true
        AND url IS NOT NULL
...
async function persist(result) {
    try {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [result.productId, result.merchantId, result.url, result.status, result.reason, result.httpStatus, result.latencyMs, result.error]);
        ...
        console.warn('[outbound-probe] url_probe_log insert failed (continuing with product status update):', err.message);
    }
