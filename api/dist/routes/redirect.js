...
const legacyUrlRedirectHandler = async (req, res) => {
    ...
    (async () => {
        try {
            await withTimeout(insertAffiliateClickWithTruth([who.keyHash, 'legacy_url', 'unknown', 'unknown', '', source, destinationUrl,
                who.ua, who.family, who.ipHash, who.referrer, who.sourcePage, who.keyId, who.isInternal], false, 302), REDIRECT_TIMEOUT_MS, 'affiliate_clicks insert (legacy url)');
        }
        catch (err) {
            if (err.code !== '42703') {
                console.warn('[redirect] legacy-url click logging failed:', err.message);
            }
        }
    })();
