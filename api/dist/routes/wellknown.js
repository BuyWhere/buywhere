...
const A2A_AGENT_CARD = {
    name: 'BuyWhere',
    description: 'Real-time shopping API and agent-native product catalog for AI agents: 381M+ products from 900,000+ merchants worldwide, with location-aware deliver_to ranking and per-result availability labels.',
    ...
    skills: [
        {
            id: 'product-search',
            name: 'Product Search',
            description: 'Search 381M+ products worldwide by keyword, category, price range, merchant, country, and deliver_to for deliverable-first ranking.',
        ...
        },
        {
            ...
            description: 'Compare products, prices, attributes, and availability across 900,000+ independent storefronts worldwide.',
    ...
    protocols: {
        mcp: {
            ...
            notes: 'Canonical MCP endpoint: JSON-RPC over HTTP POST (Streamable HTTP).',
        },
        rest: {
...
router.get('/ai-plugin.json', (_req, res) => {
    res.json({
        ...
        description_for_human: 'Product catalog for AI agents: 300M+ products from 900,000+ storefronts worldwide, normalized into one schema. Location-aware: pass deliver_to and every result carries an availability label (local | ships_to_you | unavailable).',
        description_for_model: 'Use this plugin to search the BuyWhere global product catalog for AI agents and LLM apps. Search 300M+ products across 900,000+ storefronts worldwide; pass deliver_to=<ISO country> for the end user delivery location so results rank deliverable-first and include availability labels (local | ships_to_you | unavailable | unknown). Filter by merchant/retailer, price range, country, currency, availability, and include_unshippable. Register for a free API key at https://api.buywhere.ai/v1/auth/register.',
        auth: {
            type: 'user_http',
...
router.get('/api-catalog', (_req, res) => {
    res.json({
        ...
        base_url: `${config_1.API_BASE_URL}`,
        endpoints: {
            rest: `${config_1.API_BASE_URL}/v1/products`,
            openapi: `${config_1.API_BASE_URL}/openapi.json`,
            mcp: `${config_1.API_BASE_URL}/mcp`,
...
router.get('/ai-agent.json', (_req, res) => {
