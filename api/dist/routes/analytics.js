...
router.get('/query-count', requireAdminKey, async (req, res) => {
    const days = Math.min(parseInt(req.query.days || '30'), 90);
    ...
    const [dailyResult, totalResult, endpointResult] = await Promise.all([
       ...
       WHERE ql.created_at >= NOW() - ($1 || ' days')::interval
         ...
         AND (ak.is_test IS NOT TRUE OR ak.id IS NULL)
       GROUP BY day
       ORDER BY day DESC`, [days, CORE_ENDPOINTS]),
        // Rolling totals
        config_1.db.query(`SELECT
         COUNT(*) AS total,
         COUNT(*) FILTER (WHERE ql.is_agent = true) AS agent_count,
       ...
       WHERE ql.created_at >= NOW() - ($1 || ' days')::interval
         AND ql.endpoint = ANY($2)
         AND (ak.is_test IS NOT TRUE OR ak.id IS NULL)`, [days, CORE_ENDPOINTS]),
       ...
       WHERE ql.created_at >= NOW() - ($1 || ' days')::interval
         ...
         AND (ak.is_test IS NOT TRUE OR ak.id IS NULL)
       GROUP BY ql.endpoint
       ORDER BY total DESC`, [days, CORE_ENDPOINTS]),
    ]);
...
router.get('/launch-window', requireAdminKey, async (req, res) => {
    ...
    const [totalsResult, firstQueryResult, registrationsResult, endpointResult] = await Promise.all([
       ...
       WHERE ql.created_at >= $1 AND ql.created_at < $2
         ...
         AND (ak.is_test IS NOT TRUE OR ak.id IS NULL)`, [startDate.toISOString(), endDate.toISOString(), CORE_ENDPOINTS]),
       ...
       WHERE created_at >= $1 AND created_at < $2
         AND is_test IS NOT TRUE`, [startDate.toISOString(), endDate.toISOString()]),
        // Per-endpoint breakdown
        config_1.db.query(`SELECT
         ql.endpoint,
         COUNT(*) AS total,
...
router.get('/latency', requireAdminKey, async (req, res) => {
    ...
    const [overallResult, endpointResult] = await Promise.all([
       ...
       WHERE ql.created_at >= $1
         ...
         AND (ak.is_test IS NOT TRUE OR ak.id IS NULL)`, [cutoff, CORE_ENDPOINTS]),
        config_1.db.query(`SELECT
         ql.endpoint,
         COUNT(*) AS sample_count,
         ROUND(PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY ql.response_time_ms))::int AS p50,
