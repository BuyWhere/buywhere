...
function pruneExpired(stats, cutoffMs) {
    ...
    while (stats.count > 0) {
        ...
        dropped += 1;
        // Advance head.
        stats.samples[stats.head] = undefined;
        stats.head = (stats.head + 1) % RING_BUFFER_SIZE;
        stats.count -= 1;
    }
...
function snapshotHistograms() {
    ...
    for (const [routeKey, stats] of ROUTE_STATS.entries()) {
        ...
        const latencies = new Array(stats.count);
        const buckets = exports.LATENCY_BUCKETS_MS.map((le) => ({ le_ms: le, count: 0 }));
 ...
 */
function histogramLatencyMiddleware(req, res, next) {
    // Avoid double-instrumentation when mounted twice (e.g. legacy + new wiring).
    if (req.__histogramStart !== undefined) {
        return next();
    }
    ...
    req.__histogramStart = startNs;
    res.once('finish', () => {
        const elapsedNs = process.hrtime.bigint() - startNs;
        const latencyMs = Math.round(Number(elapsedNs) / 1000000);
