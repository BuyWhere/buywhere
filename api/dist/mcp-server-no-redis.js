...
// Modified to run without Redis for testing
const express_1 = __importDefault(require("express"));
const cors_1 = __importDefault(require("cors"));
const mcp_1 = __importDefault(require("./routes/mcp"));
const config_no_redis_1 = require("./config-no-redis");
...
app.get('/health', async (_req, res) => {
    try {
        const result = await config_no_redis_1.db.query('SELECT reltuples::bigint AS count FROM pg_class WHERE oid = \'public.products\'::regclass');
        res.json({
            status: 'ok',
            server: 'mcp',
...
const server = app.listen(MCP_PORT, () => {
    ...
    console.log(`  Note: Redis is disabled for testing`);
});
