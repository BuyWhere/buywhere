...
__export(index_exports, {
  AgentSearchProductsTool: () => AgentSearchProductsTool,
  ComparePricesTool: () => ComparePricesTool,
  CompareProductsTool: () => CompareProductsTool,
  FindBestPriceTool: () => FindBestPriceTool,
  GetDealsTool: () => GetDealsTool,
  GetPriceHistoryTool: () => GetPriceHistoryTool,
  GetProductDetailsTool: () => GetProductDetailsTool,
  GetProductDetailsV2Tool: () => GetProductDetailsV2Tool,
  GetPurchaseOptionsTool: () => GetPurchaseOptionsTool,
  ResolveProductQueryTool: () => ResolveProductQueryTool,
  SearchProductsTool: () => SearchProductsTool,
  createAgentTools: () => createAgentTools,
  createBuyWhereTools: () => createBuyWhereTools,
  createGetPriceComparisonTool: () => createGetPriceComparisonTool,
  createGetProductDetailsTool: () => createGetProductDetailsTool,
  createSearchProductsTool: () => createSearchProductsTool,
  createStructuredTools: () => createStructuredTools
});
module.exports = __toCommonJS(index_exports);
var import_tools = require("@langchain/core/tools");
var import_sdk = require("@buywhere/sdk");
var import_zod = require("zod");
async function withRetry(fn, opts = {}) {
  const { retries = 3, baseDelayMs = 200, maxDelayMs = 5e3 } = opts;
  let lastError;
...
var SearchProductsTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "search_products";
    this.description = `Search the BuyWhere product catalog for products matching a query.
...
Returns product listings with prices, merchant info, availability, and affiliate links.`;
    this.client = new import_sdk.BuyWhereSDK(config.apiKey);
  }
  async _call(input) {
    try {
...
var ComparePricesTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "compare_prices";
    this.description = `Compare prices for a product across multiple merchants.
...
var GetDealsTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "get_deals";
    this.description = `Get current deals and price drops from BuyWhere.
...
var GetProductDetailsTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "get_product_details";
    this.description = `Get detailed information about a specific product by its ID.
...
var GetPriceHistoryTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "get_price_history";
    this.description = `Get historical price data for a product to track price trends.
...
var AgentSearchProductsTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "agent_search_products";
    this.description = `Agent-optimized product search using BuyWhere v2 API with natural language processing.
...
function createBuyWhereTools(config) {
  return [
...
var ResolveProductQueryTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "resolve_product_query";
    this.description = `Use this whenever a user asks to find, search, or look up products \u2014 especially when they want product recommendations or need to discover options for a shopping decision.
...
var FindBestPriceTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "find_best_price";
    this.description = `Use this whenever a user asks about prices, wants to find the cheapest option, or asks "what's the best price for X" or "where can I buy X for the lowest price".
...
var CompareProductsTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "compare_products";
    this.description = `Use this whenever a user wants to compare multiple products, see side-by-side price comparisons, or understand the differences between product options.
...
var GetProductDetailsV2Tool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "get_product_details_v2";
    this.description = `Use this whenever a user wants detailed information about a specific product, needs to see all available prices from different merchants, or wants to see product reviews and price history.
...
var GetPurchaseOptionsTool = class extends import_tools.Tool {
  constructor(config) {
    super();
    this.name = "get_purchase_options";
    this.description = `Use this whenever a user is ready to buy and wants to see all purchase options, needs merchant choices for a product, or wants to compare fulfillment ratings and reliability across merchants.
...
var COUNTRY_CODES = ["SG", "MY", "TH", "PH", "VN", "ID", "US"];
function createSearchProductsTool(config) {
  const client = new import_sdk.BuyWhereSDK(config.apiKey);
  return new import_tools.DynamicStructuredTool({
    name: "search_products",
    description: "Search the BuyWhere product catalog using keywords or natural language. Returns matching products with title, price, availability, merchant, and affiliate URL.",
    schema: import_zod.z.object({
      query: import_zod.z.string().describe('Keyword or natural-language query (e.g. "wireless earbuds under $50")'),
      country: import_zod.z.enum(COUNTRY_CODES).optional().describe("Country code to scope results"),
      limit: import_zod.z.number().int().min(1).max(50).default(10).optional().describe("Max results to return"),
      price_min: import_zod.z.number().optional().describe("Minimum price filter"),
      price_max: import_zod.z.number().optional().describe("Maximum price filter")
    }),
...
function createGetProductDetailsTool(config) {
  const client = new import_sdk.BuyWhereSDK(config.apiKey);
  return new import_tools.DynamicStructuredTool({
    name: "get_product_details",
    description: "Get full details for a specific product by its BuyWhere product ID.",
    schema: import_zod.z.object({
      product_id: import_zod.z.number().int().describe("The unique BuyWhere product ID")
    }),
...
function createGetPriceComparisonTool(config) {
  const client = new import_sdk.BuyWhereSDK(config.apiKey);
  return new import_tools.DynamicStructuredTool({
    name: "get_price_comparison",
    description: "Compare prices for a product across all available merchants, sorted cheapest first. Use this to find the best deal for a specific product.",
    schema: import_zod.z.object({
      query: import_zod.z.string().describe("Product name or query to compare prices for"),
      category: import_zod.z.string().optional().describe('Category slug filter (e.g. "electronics")'),
      limit: import_zod.z.number().int().min(1).max(50).default(10).optional().describe("Max results to return")
    }),
...
function createStructuredTools(config) {
  return [
...
0 && (module.exports = {
  ...
  GetProductDetailsV2Tool,
  GetPurchaseOptionsTool,
  ResolveProductQueryTool,
  SearchProductsTool,
  createAgentTools,
  createBuyWhereTools,
  createGetPriceComparisonTool,
