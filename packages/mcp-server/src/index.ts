...
if (!API_KEY) {
  process.stderr.write(
    "[buywhere-mcp] Warning: BUYWHERE_API_KEY is not set. Tool calls will fail.\n"
  );
}

async function callHostedMcp(
  toolName: string,
...
server.tool(
  "search_products",
  "Search BuyWhere's normalized product catalog by keyword, category, price range, merchant domain, or region. Returns ranked product records with prices, merchants, and affiliate URLs.",
  {
  ...
  async (args) => {
    const result = await callHostedMcp("search_products", args as Record<string, unknown>);
...
server.tool(
  "get_product",
  "Retrieve full product details from BuyWhere by product ID, including price history, merchant offers, availability, and affiliate URLs.",
  {
    id: z.string().describe("BuyWhere product ID"),
  },
  async (args) => {
    const result = await callHostedMcp("get_product", args as Record<string, unknown>);
    return { content: [{ type: "text", text: toText(result) }] };
  }
);
...
server.tool(
  "compare_products",
  "Side-by-side comparison of 2 to 5 BuyWhere products. Returns a structured comparison of prices, merchants, specifications, and availability across multiple retailers.",
  {
    ids: z.array(z.string()).describe("Array of 2–5 BuyWhere product IDs to compare"),
  },
  async (args) => {
    const result = await callHostedMcp("compare_products", args as Record<string, unknown>);
    return { content: [{ type: "text", text: toText(result) }] };
  }
);
...
server.tool(
  "get_deals",
  "Get current deals, price drops, and promotions from BuyWhere's catalog across Singapore and Southeast Asia markets.",
  {},
  async (_args) => {
...
server.tool(
  "list_categories",
  "List the available product category taxonomy in BuyWhere's catalog. Use this to discover valid category names for search_products filters.",
  {},
  async (_args) => {
